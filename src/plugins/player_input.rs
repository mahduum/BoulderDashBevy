...
/* every tile is an entity, but if I transfer from entity to entity,
 state only depends on delta so it will operate on all the components that have Delta Component and State Component
 and it will update those components accordingly - the system will produce new state and then it will assign it only if it has changed
 but when copied/cloned it would have to be cloned with all the garbage, each state will have to implement get animation name method */
...
fn setup(mut commands: Commands){
    //commands.insert_resource(InputDelayTimer(Timer::from_seconds(0.3, Repeating)));
}
...
fn keyboard_input(
    ...
    mut timer: ResMut<InputDelayTimer>,
    time: Res<Time>,
    ...
    if timer.tick(time.delta()).just_finished() == false{
        return;
    }
    ...
    if delta == Delta::zero() && keyboard.pressed(KeyCode::D) {
        //println!("Pressed D");
        delta.x = 1;
    }

    if delta == Delta::zero() && keyboard.pressed(KeyCode::W) {
        //println!("Pressed W");
        delta.y = 1;
    }

    if delta == Delta::zero() && keyboard.pressed(KeyCode::S) {
        //println!("Pressed S");
        delta.y = -1;
    }
    ...
    match player_query.get_single_mut() {
       Ok(entity) => {commands.entity(entity).insert(delta);}
